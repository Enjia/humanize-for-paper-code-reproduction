...
"
...

    {
        ...
        echo ""
        echo "codex review $(bash3_join_args "${CODEX_DISABLE_HOOKS_ARGS[@]+"${CODEX_DISABLE_HOOKS_ARGS[@]}"}" --base "$review_base" "${CODEX_REVIEW_ARGS[@]}")"
...
CODEX_PROMPT_CONTENT=$(cat "$REVIEW_PROMPT_FILE")
{
    ...
    echo "# Timeout: $CODEX_TIMEOUT seconds"
    echo ""
    echo "codex exec $(bash3_join_args "${CODEX_DISABLE_HOOKS_ARGS[@]+"${CODEX_DISABLE_HOOKS_ARGS[@]}"}" "${CODEX_EXEC_ARGS[@]}") \"<prompt>\""
    echo ""
    echo "# Prompt content:"
